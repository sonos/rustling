...
where
    V: Value + ::std::fmt::Debug + StashIndexable,
    RuleId: ClassifierId,
    Feat: Feature,
    Extractor: FeatureExtractor<V, Feat>,
{
    ...
    pub fn candidates<Tagger: MaxElementTagger<V>>(
        &self,
        input: &str,
        tagger: &Tagger,
    ) -> RustlingResult<Vec<Candidate<V, Tagger::O>>> {
        Ok(tagger.tag(self.raw_candidates(input)?))
    }

    pub fn parse<Tagger: MaxElementTagger<V>>(
        &self,
        input: &str,
        tagger: &Tagger,
    ) -> RustlingResult<Vec<ParserMatch<Tagger::O>>> {
        Ok(self
            .candidates(input, tagger)?
            .into_iter()
            .filter_map(|c| if c.tagged { Some(c.match_) } else { None })
            .collect())
    }

    pub fn analyse<Tagger: MaxElementTagger<V>>(
        &self,
        examples: Vec<&str>,
        tagger: &Tagger,
        ...
        Ok(ParsingAnalysis {
            ...
            text_pattern_coverage: 1.0
                - (unused_text_pattern.len() as f32 / text_pattern_syms.len() as f32),
...
mod tests {
    ...
    fn test_integer_numeric_infix_rule() {
        ...
        assert_eq!(
            vec![Int(12), Int(42), Int(12), Int(42), Int(54), Int(504)],
            values
        );
    }
    ...
    impl FeatureExtractor<MyValue, TestFeat> for TestFeatExtractor {
        fn for_parsed_node(&self, node: &ParsedNode<MyValue>) -> Input<RuleId, TestFeat> {
    ...
    impl MaxElementTagger<MyValue> for TestMaxElementTagger {
        ...
        fn tag(
            &self,
            candidates: Vec<(ParsedNode<MyValue>, ParserMatch<MyValue>)>,
        ) -> Vec<Candidate<MyValue, MyValue>> {
            let mut candidates = candidates;
            candidates.sort_by(|a, b| a.1.byte_range.len().cmp(&b.1.byte_range.len()));
            candidates
                .into_iter()
                .rev()
                .enumerate()
    ...
    fn parser() -> Parser<MyValue, TestFeat, TestFeatExtractor> {
        Parser {
            ...
            model: Model {
                classifiers: FnvHashMap::default(),
            },
    ...
    fn test_parsing_analysis() {
        ...
        assert_eq!(
            ...
            parser
                .analyse(
                    vec!["example that should fail", "foo: 1.5^2", "foo: 2"],
                    &TestMaxElementTagger
                )
                .unwrap()
        );
