...
use crate::{
    AttemptFrom, CoreResult, InnerStashIndexable, Node, NodePayload, ParsedNode, ParsingStatus,
    SendSyncPhantomData, Stash, StashIndexable, Sym,
};
...
pub trait TerminalPattern<StashValue: NodePayload + StashIndexable>:
    Pattern<StashValue, M = Text<StashValue>>
{
}
...
impl<StashValue: NodePayload + StashIndexable> TerminalPattern<StashValue>
    for TextPattern<StashValue>
{
}
...
impl<StashValue: NodePayload + StashIndexable> Pattern<StashValue>
    for TextNegLHPattern<StashValue>
{
    type M = Text<StashValue>;
    fn predicate(
        &self,
...
where
    StashValue: NodePayload + StashIndexable,
    V: NodePayload<Payload = StashValue::Payload>
        + InnerStashIndexable<Index = StashValue::Index>
        + AttemptFrom<StashValue>,
{
    type M = ParsedNode<V>;
    fn predicate(
        &self,
...
mod tests {
    ...
    fn test_regex_separated_string() {
        ...
        let pat: TextPattern<usize> =
            TextPattern::new(::regex::Regex::new("a+").unwrap(), Sym(0), checker);
        assert_eq!(
            vec![Text::new(svec4!(Range(0, 3)), Range(0, 3), Sym(0))],
        ...
        assert_eq!(
            vec![Text::new(svec4!(Range(4, 7)), Range(4, 7), Sym(0))],
            pat.predicate(&stash, "bbb aaa").unwrap().matches
        );
        assert_eq!(
            Vec::<Text<usize>>::new(),
            pat.predicate(&stash, "baaa").unwrap().matches
        );
        assert_eq!(
            Vec::<Text<usize>>::new(),
            pat.predicate(&stash, "aaab").unwrap().matches
        );
        assert_eq!(
            Vec::<Text<usize>>::new(),
            pat.predicate(&stash, "aaaé").unwrap().matches
        );
        assert_eq!(
            Vec::<Text<usize>>::new(),
            pat.predicate(&stash, "éaaa").unwrap().matches
        );
        assert_eq!(
            vec![Text::new(svec4!(Range(1, 4)), Range(1, 4), Sym(0))],
